...
void PX4CtrlFSM::process()
{
    ...
    case MANUAL_CTRL:
    {
        if (rc_dy_data.enter_hover_mode) // Try to jump to AUTO_HOVER
        {
            ...
            state = AUTO_HOVER;
            controller.resetThrustMapping();    //g/percent_hover 计算推力模型 悬停
            set_hov_with_odom();
    ...
    if (rotor_low_speed_during_land) {// used at the start of auto takeoff
        ...
        debug_msg = controller.calculateControl(des, odom_data, imu_data, u);   //PID计算角度
    ...
    if (param.mpc_ctrl) {
        ...
        publish_mpc_ctrl(des, now_time);//TODO:u改成mpc输出的控制量
    } else {
...
void PX4CtrlFSM::publish_mpc_ctrl(const Desired_State_t &des, const ros::Time &stamp) {
    ...
    pos.velocity.x = des.v.x();
    pos.velocity.y = des.v.y();
    pos.velocity.z = des.v.z();

    pos.yaw = 0.0f;
    pos.yaw_rate = des.yaw_rate;

    ctrl_mpc_pub.publish(pos);
}
...
void PX4CtrlFSM::publish_trigger() {
